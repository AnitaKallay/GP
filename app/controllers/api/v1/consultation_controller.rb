...
before_action :set_consultation, except: [:create, :index]
  ...
  def index
   @pagy, @consultations = pagy(ConsultationService.new(params).filter_proces)
    render json: {consultations: @consultations.map{|item|ConsultationListSerializer.new(item,@current_user)},total_page: @pagy.page}
  end

  def create
   @consultation = Consultation.new(cons_params)
     if @consultation.save
       render json: {consultation: ConsultationSerializer.new(@consultation,@current_user)}
     else
